    ...
    for color in Color::ALL {
        ...
        for piece in Piece::ALL {
            ...
            for square in piece_occupancy.iter_ones() {
                ...
                color_eval += (e2 - e1) * v.end_game_weight + e1;
            }
        }

        evaluation += color_eval * if color == perspective { 1.0 } else { -1.0 };
    }

    *eval += Evaluation::from((evaluation * BASE_WEIGHT) as i32);
}
...
mod tests {
    ...
    #[test]
    fn test_pawn_map() {
        let evaluator = Evaluator::just(&[super::evaluate]);
